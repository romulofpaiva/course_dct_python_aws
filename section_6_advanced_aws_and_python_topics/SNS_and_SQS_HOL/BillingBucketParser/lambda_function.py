...
# make a fake api call to get international taxes, rasing a error that changes the lambda_handler flow 
def get_international_taxes(valid_product_lines, billing_bucket, csv_file):
    try:
        ...
        # Catch error and publish to SNS topic.
        sns_topic_arn = 'arn:aws:sns:us-east-1:568533826271:TaxesAPIConnectionError'
        message = f"Lambda function failed to reach International Taxes for '{billing_bucket}' bucket and file '{csv_file}'. Error: '{error}'"
        sns.publish(TopicArn=sns_topic_arn, Message=message, Subject='Lambda API Call Failure')
    
    # Raise error to propagate to the calling function.
    print("Published failutre to SNS topic.")
    raise error
...
def lambda_handler(event, context):
    ...
    # Call the get_international_taxes function to simulate an API error.
    try:
